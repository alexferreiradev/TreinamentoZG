...
class HandUtil {
     ...
     */
   static int countSameCardValue(Card card, Enumeration<Card> cards) {
        int count = 0

        while (cards.hasMoreElements()){
            Card cardComparing = cards.nextElement()
            if (card.valueType == cardComparing.valueType){
                count ++
            }
        }
   ...
   static Map<CardValue, Integer> groupOfValues(List<Card> cards) {
   ...
   static Card getLastCard(List<Card> cards) {
  ...
  static Card getNextCard(List<Card> cards, Card currentCard) {
     ...
     */
   static boolean isHigherModel(Queue<? extends ComparableModel> baseModels, Queue<? extends ComparableModel> comparableModels) {
   ...
   static boolean isSameModel(Queue<? extends ComparableModel> baseModels, Queue<? extends ComparableModel> comparableModels) {
